(function () {
  ...
  const form = document.getElementById('titanymContactForm');
  ...
  /* ── Gauge needle — animate when scrolled into view ────── */
  const needle = document.getElementById('gauge-needle');
  if (needle) {
    const gaugeObs = new IntersectionObserver(function (entries) {
      entries.forEach(function (entry) {
        if (entry.isIntersecting) {
          needle.classList.add('animated');
          gaugeObs.unobserve(entry.target);
        }
      });
    }, { threshold: 0.5 });
    gaugeObs.observe(needle);
  }
